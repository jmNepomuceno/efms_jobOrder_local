...
try {
    $sql = "SELECT requestNo, requestDate, requestBy, requestDescription, requestCategory, requestStatus, processedBy, requestPendingMaterialsDate, requestCorrection , processedByID, assignTo, assignToBioID, assignTargetStartDate, assignTargetEndDate, requestPendingMaterials
    FROM job_order_request 
    WHERE CAST(JSON_EXTRACT(requestBy, '$.bioID') AS UNSIGNED) = ? 
    AND requestStatus = 'Pending Materials'";
